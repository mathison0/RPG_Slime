 ...
 * 서버용 닌자 직업 클래스
 */
class NinjaJob extends BaseJob {
    constructor(player) {
        super(player);
        this.jobInfo = getJobInfo('ninja');
    ...
    useSkill(skillType, options = {}) {
        switch (skillType) {
            ...
            default:
                console.log('NinjaJob: 알 수 없는 스킬 타입:', skillType);
    ...
    useStealth(options = {}) {
        ...
        console.log(`닌자 은신 발동! 지속시간: ${skillInfo.duration}ms`);
    ...
    useBasicAttack(targetX, targetY, options = {}) {
        ...
        console.log(`닌자 기본 공격 발동! 데미지: ${damage}`);
    ...
    update(delta) {
        ...
        if (this.player.isStealth) {
            ...
            if (now - this.player.stealthStartTime >= this.player.stealthDuration) {
                ...
                this.player.stealthStartTime = 0;
                this.player.stealthDuration = 0;
                console.log('닌자 은신 자동 해제');
            }
        }
    }
}

module.exports = NinjaJob; 