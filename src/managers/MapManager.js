...
export default class MapManager {
    constructor(scene) {
        ...
        this.playerSpawnBarrierCollider = null;
    ...
    createMapElements(mapData) {
        ...
        this.scene.spawnBarriers = this.scene.physics.add.staticGroup();
    ...
    createSpawnBarriers() {
        ...
        if (this.scene.player.team === 'red') {
            ...
            this.scene.spawnBarriers.create(
                this.scene.blueSpawnRect.x - barrierThickness / 2, 
                this.scene.blueSpawnRect.centerY, 
                null
            ).setSize(barrierThickness, this.scene.blueSpawnRect.height)
             .refreshBody()
             .setVisible(false);
            ...
            this.scene.spawnBarriers.create(
                this.scene.redSpawnRect.right + barrierThickness / 2, 
                this.scene.redSpawnRect.centerY, 
                null
            ).setSize(barrierThickness, this.scene.redSpawnRect.height)
             .refreshBody()
             .setVisible(false);
        }
    ...
    setupCollisions() {
        ...
        // 스폰 구역 물리적 장벽 충돌
        if (this.scene.spawnBarriers) {
            this.playerSpawnBarrierCollider = this.scene.physics.add.collider(
                this.scene.player, 
                this.scene.spawnBarriers
            );
        }
    ...
    pushPlayerOutOfWall(collidingWall) {
        if (!this.scene.player || !collidingWall) {
            return;
        }
